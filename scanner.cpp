...
Token* Scanner::nextToken() {
    ...
    if (isdigit(c)) {
       ...
       while (current < input.length() && input[current] != '"'){
        ...
        if (input[current] == '"') {
            current++;
            token = new Token(Token::STR, input, first + 1, current - first - 2);
        } else {
        ...
        if (word == "print") {
            ...
            token = new Token(Token::END_FOR, word, 0, word.length());
        }
        ...
        switch(c) {
            ...
            case '[': token = new Token(Token::CORCHETE_I, c); break;
            case ']': token = new Token(Token::CORCHETE_D, c); break;
